...
import xarray as xr
import xesmf
from regrid_GLORYS_para import regrid_GLORYS
...
#   src_angle = regrid_GLORYS(src_grd.angle, method='bilinear', irange=irange, jrange=jrange)
    ...
    angle = dst_angle
    #np.save('/Volumes/homes/HauriLab/ROMS-GOAF/test/angle.npy',np.array(angle))
    #np.save('/Volumes/homes/HauriLab/ROMS-GOAF/test/dst_u_south.npy',np.array(dst_u_south))
    #np.save('/Volumes/homes/HauriLab/ROMS-GOAF/test/dst_v_south.npy',np.array(dst_v_south))
    print('dst_grd.vgrid.N',dst_grd.vgrid.N)
